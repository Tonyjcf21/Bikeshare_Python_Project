...
def get_filters():
    ...
    while True:  # Lesson 7.13
        ...
        try:
            city = str(
                input(
                    "Please choose the city (chicago, new york city or washington) that you'd like to analyze: "
                )
            ).lower()
            print("\n")
        except:
        ...
        if city == "new york":
            ...
            print(
                "Please type a valid input. That is: chicago, new york city or washington."
            )
            print("\n")
            continue
        else:
    ...
    while True:
        try:
            month = str(
                input(
                    "Please choose a month to filter by or type 'all' if you don't want a filter: "
                )
            ).lower()
            print("\n")
        except:
        ...
        if month in invalid_months:
            print(
                "Sorry, the data only goes from january to june. Select a month within that range"
            )
            print("\n")
            continue
        elif month not in valid_months:
            print(
                "Please type a valid input. That is: a month between january and june."
            )
            print("\n")
            continue
        else:
    ...
    while True:
        try:
            day = str(
                input(
                    "Choose a day of the week or type 'all' if you don't want a filter: "
                )
            ).lower()
            print("\n")
        except:
...
def load_data(city, month, day):
    ...
    df["day_of_week"] = df[
        "Start Time"
    ].dt.day_name()  # .weekday_name gave me this error:
...
def time_stats(
    month, day, df
):  # I needed to add month and day to the arguments for my code to work because I'm using those values as conditions.
    ...
    if month == "all":
        ...
        valid_months = ["january", "february", "march", "april", "may", "june"]
        popular_month = popular_month.set_axis(
            valid_months
        )  # https://stackoverflow.com/a/69336374

        list_of_popular_months = []
        sorted_pop_month = popular_month.sort_values(
            ascending=False
        )  # https://pandas.pydata.org/docs/reference/api/pandas.Series.sort_values.html
        flag = sorted_pop_month[
            0
        ]  # Here we neeeded to convert months to letters again or flag[0] would be month 0, which doesnt exist!

        list_append(flag, popular_month, list_of_popular_months)

        print(
            "The most common month(s) to travel according to the data selected is/are: "
        )
    ...
    if day == "all":
        ...
        sorted_pop_day = popular_day.sort_values(
            ascending=False
        )  # https://pandas.pydata.org/docs/reference/api/pandas.Series.sort_values.html
        ...
        print(
            "The most common day(s) of the week to travel according to the data selected is/are: "
        )
    ...
    flag = sorted_pop_hour.iloc[
        0
    ]  # .iloc was needed here because, without it, we get the hour 0
    # and not the first index of the Series.

    list_append(flag, popular_hour, list_of_popular_hours)

    print(
        "The most common hour(s) of the day to travel according to the data selected is/are: "
    )
...
def station_stats(df):
    ...
    if (
        flag == 1
    ):  # This obviously never happens for a large enough dataset, but I was using the 400
        # row version and sometimes it did so... one can never be too careful.
        print(
            "For the selected filters, every occurrence only happens once. Therefore, the is not \
    ...
    print(
        "The most common combination of starting-end stations, according to the data selected is/are: "
    )
...
def trip_duration_stats(df):
    ...
    print(
        "The total duration for trips selected by your filters is:\n {} hours {} minutes and {} seconds".format(
            int(total_hours_int_part),
            int((total_hours_decimal_part * 60) // 1),
    ...
    print(
        "The average duration for a trip in your selected data is:\n {} hours {} minutes and {} seconds".format(
            int(total_int_avg),
            int((total_dec_avg * 60) // 1),
            int(((total_dec_avg * 60) % 1) * 60),
        )
    )
...
def user_stats(city, df):
    ...
    if city != "washington":
        ...
        print(
            "The year of birth of the earliest person is: {}".format(
                int(recent_byear.index[-1])
            )
        )
        print(
            "The year of birth of the most recent person is: {}".format(
                int(recent_byear.index[0])
            )
        )
        print(
            "The most common year of birth is: {} with {} people that were born that year for the filters applied.".format(
                int(common_byear.index[0]), common_byear.iloc[0]
            )
        )
...
def main():
    while True:
        ...
        try:
            raw = str(
                input(
                    "Before we start the statistics, would you like to see raw data from your selection? Please answer 'yes' or 'no'.\n-------->"
                )
            )
        except:
